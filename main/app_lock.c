...
void app_lock_init(lv_obj_t *screen)
{
    ...
    s_clock = lv_label_create(s_box);
    lv_obj_set_style_text_font(s_clock, ui_pixel_font_20(), 0);
    lv_obj_set_style_text_color(s_clock, lv_color_hex(0xFFFFFF), 0);
    lv_obj_align(s_clock, LV_ALIGN_TOP_MID, 0, 100);

    s_icon = lv_obj_create(s_box);
    ui_pixel_strip_theme(s_icon);
    lv_obj_set_size(s_icon, 40, 40);
    lv_obj_align(s_icon, LV_ALIGN_TOP_MID, 0, 140);
    ...
    lv_obj_align(s_wx, LV_ALIGN_TOP_MID, 0, 188);
