...
static const char PAGE[] =
    ...
    "let ws,ac,seq=0,talk=0,ch=1,encS={p:0,i:0},proc,src,micE='',playT=0;"
    "const ptt=document.getElementById('ptt');"
    "const st=document.getElementById('st');"
    "function play(pcm){if(!pcm||!pcm.length)return;ac=ac||new AudioContext();"
    "const b=ac.createBuffer(1,pcm.length,8000);const d=b.getChannelData(0);"
    "for(let i=0;i<pcm.length;i++){let v=pcm[i]/32768;if(i<8)v*=(i+1)/8;"
    "if(i>pcm.length-9)v*=(pcm.length-i)/8;d[i]=v}"
    "const n=ac.createBufferSource();n.buffer=b;n.connect(ac.destination);"
    "const t=ac.currentTime;if(playT<t+0.02)playT=t+0.02;n.start(playT);playT+=b.duration}"
    ...
    "let stream;try{stream=await g({audio:{echoCancellation:true,"
    "noiseSuppression:true,autoGainControl:true},video:false})}"
    "catch(e){stream=await g({audio:true,video:false})}"
    ...
    "for(let i=0;i<160;i++){const x0=i*ratio,x1=x0+ratio;let s=0,c=0;"
    "const j1=Math.min(acc.length,(x1|0)+1);for(let j=x0|0;j<j1;j++){s+=acc[j];c++}"
    "const v=c?s/c:0;pcm[i]=Math.max(-32768,Math.min(32767,v*32767))}"
    "acc=acc.subarray(Math.floor(ratio*160));"
    "const snap={p:encS.p,i:encS.i};const ad=enc(encS,pcm);"
    "ws.send(pack(ch,seq++&255,5,snap,ad))}};"
    "src.connect(proc);const z=ac.createGain();z.gain.value=0;"
    "proc.connect(z);z.connect(ac.destination);}"
