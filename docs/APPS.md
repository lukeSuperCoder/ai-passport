...
If the clock is valid (NTP or Date & Time in Settings), the pet sleeps in the window set under **Sleep hours** (**21:00–08:00** by default). Same hour for bedtime and wake turns scheduled sleep off. At bedtime, if the lights are still on or care is still in the danger range, the device chimes **It's bedtime** (and wakes from panel sleep). Opening Home while it is bedtime and the lights are still on repeats that toast. Feed, clean, and heal still work until you turn the lights off; after that, care except Light is refused with **sleeping now**. Minigames still work. Bag Trip, Visit, and Battle need an awake pet. Leave the lights off at night or mood suffers.
